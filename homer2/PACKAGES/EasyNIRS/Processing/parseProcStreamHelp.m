function procFunc = parseProcStreamHelp(procFunc,externVars)

if ~exist('externVars','var')
    externVars={};
end

funcReg.call = procStreamReg();
funcReg.help = procStreamRegHelp();
for ii=1:length(funcReg.call)
    procInputReg = parseProcessOpt(funcReg.call{ii},externVars);
    ...
    for jj=1:procInputReg.procFunc.nFunc
        if strcmp(procFunc.funcName{iFunc},procInputReg.procFunc.funcName{jj})
            ...
            if funcMatch(f1,f2)
                ...
                procFunc.funcHelp{iFunc} = parseFuncHelp(procInputReg.procFunc, jj, ...
                                                         externVars);
